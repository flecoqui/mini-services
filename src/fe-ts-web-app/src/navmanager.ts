...
export class NavigationManager {
  ...
  initializeClicks() {
    let navbarDiv: HTMLDivElement = (<HTMLDivElement>document.getElementById(this.navId));
    if (navbarDiv) {
      let links: HTMLCollection = navbarDiv.getElementsByTagName("button");;
      if (links) {
        let i: number;
        for (i = 0; i < links.length; i++) {
          let link: Element = links[i];
          if (link) {
            let linkName: string | null = link.getAttribute("href");
            if (linkName) {
              console.log(`HREF: ${linkName}`);
              let pageName: string = linkName.substring(1);
              if (this.pageMap.has(pageName)) {
                (link as HTMLElement).addEventListener("click", (e: Event) => {
                  let config = this.pageMap.get(pageName);
                  if ((config) && (config.pageNavigateFunction)) {
                    console.log(`Event click for page: ${pageName}`);
  ...
  async getPageContentAsync(pageId: string) {
    ...
    if (this.partialsHTMLCache[pageId]) {
      ...
      let config = this.pageMap.get(pageId);
    ...
    let contentDiv: HTMLDivElement = (<HTMLDivElement>document.getElementById(this.childId));
    if (contentDiv) {
      if (content) {
  ...
  setActiveLink(pageId: string): void {
    ...
    let navbarDiv: HTMLDivElement = (<HTMLDivElement>document.getElementById("navbarsExampleDefault"));
    if (navbarDiv) {
      let links: HTMLCollection = navbarDiv.getElementsByTagName("button");;
      if (links) {
          ...
          if (link) {
            ...
            if (linkName) {
              ...
              if (pageName === pageId) {
                ...
                // check for focus
                var isFocused = (document.activeElement === link);
                if (isFocused != true) {
  ...
  isChildPage(pagePath: string): boolean {
    for (let value of Array.from(this.pageMap.values())) {
  ...
  async navigate() {
    ...
    if (!this.isPageAllowed(pageId)) {
      var newPage = this.getFirstPageAllowed();
      if (newPage) {
  ...
  HideBurgerMenu(): void {
    var button: HTMLButtonElement = (<HTMLButtonElement>document.getElementById(this.burgerId));
    if (button) {
      button.classList.add("collapsed");
    }
    var nav = (<HTMLDivElement>document.getElementById(this.navId));
    if (nav) {
